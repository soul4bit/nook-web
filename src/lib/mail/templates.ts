...
Перед входом подтвердите email по ссылке из письма подтверждения.
Страница входа:
${url}
...
Если это не вы, ответьте на это письмо или свяжитесь с администратором.`,
    html: buildShell({
      ...
      body: `${greeting} Ваша заявка для <strong>${escapeHtml(
        email
      )}</strong> одобрена. Перед входом подтвердите email по ссылке из отдельного письма подтверждения.`,
      actionLabel: "Открыть страницу входа",
      actionUrl: url,
      footnote:
        "Если письмо подтверждения не пришло, используйте кнопку повторной отправки на странице входа.",
    }),
  };
}
